...
if [[ -z "$REPO" || -z "$DIR" ]]; then
  echo "Användning: bash tooling/scripts/bootstrap-local-first.sh --repo <git-url> --dir <path> [--user <email>]" >&2
  exit 1
fi
...
if [[ ! -f data/prisma/schema.sqlite.prisma ]]; then
...
if [[ ! -f "$DB_FILE" ]]; then
  ...
  DATABASE_URL="file:$DB_FILE" yarn prisma db push \
    --schema data/prisma/schema.sqlite.prisma \
    --accept-data-loss \
    --skip-generate
