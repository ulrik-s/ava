...
async function main(): Promise<void> {
  ...
  if (args.backend === "postgres") {
    createPostgresTarget(); // kastar (stub) tills ADR 0001 Fas 3
    return;
  }
  rmSync(args.outDir, { recursive: true, force: true });
  mkdirSync(args.outDir, { recursive: true });
  const result = await generateInto(args.outDir);
  gitCommit(args.outDir);
  console.log(`[demo-generator] backend=${args.backend} →`, result);
  console.log(`[demo-generator] git-repo: ${args.outDir}`);
}
