...
#   6. (Optional) docker compose down
#
# Användning: bash scripts/test-tier3-smoke.sh [--keep-up]
...
# ─── 1. SSH-nyckel ─────────────────────────────────────────────
bold "[1/5] SSH-nyckel"
KEYS_FILE="docker/git-ssh/authorized_keys"
if [[ ! -s "$KEYS_FILE" ]]; then
...
DEMO_BASE_PATH=/ava bash scripts/build-demo.sh >/dev/null 2>&1 || fail "build-demo.sh misslyckades"
ok "out/ skapad ($(find out -name "*.html" | wc -l | tr -d ' ') html-filer)"

# ─── 3. Docker compose up ──────────────────────────────────────
bold "[3/5] docker compose up -d --build"
docker compose up -d --build 2>&1 | grep -E "Container.*(Created|Started)" || true
ok "stack startad"
...
if [[ $KEEP_UP -eq 0 ]]; then
  ...
  echo
  ok "Stack:n är fortfarande uppe. Stoppa med: docker compose down"
fi
