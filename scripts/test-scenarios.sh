...
# tunna prod-stacken (nginx + sshd). Använd docker-compose.dev.yml.
docker compose -f docker-compose.dev.yml up -d postgres meilisearch tika

bold "[2/4] Väntar på Postgres"
for i in {1..30}; do
  if docker compose -f docker-compose.dev.yml exec -T postgres pg_isready -U ava >/dev/null 2>&1; then
    ok "postgres redo"
    break
  fi
  sleep 2
...
ok "schema synkat"
yarn tsx scripts/seed-scenario-data.ts | tail -6
ok "seed klar"

bold "[4/4] Playwright scenario-tester"
yarn playwright test --config config/playwright.scenarios.config.ts "$@"
