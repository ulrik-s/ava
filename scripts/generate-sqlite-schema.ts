/**
 * Generate `prisma/schema.sqlite.prisma` från `prisma/schema.prisma`
 * genom att byta provider och anpassa typer som inte stöds av SQLite.
 *
 * Användning (operatör vid Tauri-byggets engångs-setup):
 *
 *     yarn tsx scripts/generate-sqlite-schema.ts
 *     yarn prisma generate --schema prisma/schema.sqlite.prisma
 *     DATABASE_URL="file:./ava.db" yarn prisma db push --schema prisma/schema.sqlite.prisma
 *
...
function main(): void {
  ...
  console.log("    DATABASE_URL=\"file:./ava.db\" yarn prisma generate --schema prisma/schema.sqlite.prisma");
  console.log("    DATABASE_URL=\"file:./ava.db\" yarn prisma db push --schema prisma/schema.sqlite.prisma");
}
