...
5. **Server-first (deploy E2E)** — bygger server-first-binären, kör som docker-container mot Postgres, synkar push/pull + dokument-pipeline + **faktureringskedjan** över HTTP. Faktureringsdelen (`billing-pipeline-e2e.ts`) täcker tid → slutfaktura (frysning, moms, OCR), avbetalning enligt plan (`INSTALLMENT_PLAN` → `PAID` + `COMPLETED`) och avbetalning som inte kommer i tid (DUE → OVERDUE, idempotent scan). Försenings-delen styrs av `asOf` i st.f. väggklockan, så jobbet är datum-oberoende. Kör lokalt med `bun run e2e:billing`. **Scenario-E2E:n** (`billing-scenarios-e2e.ts`, `bun run e2e:billing-scenarios`) lägger till byråns svårare fall: brottmål/familjemål med rättshjälp resp. rättsskydd där DOMSTOL/FÖRSÄKRING och KLIENTEN delar notan (två betalande, summan måste möta hela anspråket), samt en avbetalningsplan där posterna sätts ned till hälften och ibland uteblir helt. **Fortnox-bokföringen ligger utanför PR-matrisen men i det ordinarie flödet** (#1035): workflow:et *Fortnox E2E* körs på push till `main` och nattligt, mot en egen verifikatserie och ett eget räkenskapsår. Fortnox API kan inte ta bort ett verifikat, men GUI:t kan ta bort det sista i en serie och radera ett räkenskapsår som bara innehåller manuella verifikat — därför är CI-skräpet städbart. `withBookingWindow` avvisar allt daterat utanför CI-året innan det når nätet. Att den inte ligger på PR-commits beror på att secrets inte når fork-PR:er och att `concurrency` annars serialiserar matrisen; se [`docs/fortnox-e2e.md`](./fortnox-e2e.md).
