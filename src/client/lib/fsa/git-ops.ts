...
import { DEFAULT_CORS_PROXY } from "@/client/lib/sync/cors-proxy";
 ...
 *     iso-git anropar servern direkt). Se `@/client/lib/sync/cors-proxy`.
...
export async function stageAllAndCommit(
  ...
  if (args.sshSigning) {
    ...
    const { signGitCommit } = await import("@/client/lib/keys/sign-commit");
