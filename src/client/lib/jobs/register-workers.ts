...
jobQueue.registerWorker<ExtractTextPayload>("extract-text", async (payload, ctx) => {
  ...
  ctx.setProgress(0.1);
  const { loadHandle, isFsaSupported } = await import("@/client/lib/fsa/handle-store");
  if (!isFsaSupported()) {
  ...
  ctx.setProgress(0.4);
  const { extractText } = await import("@/client/lib/jobs/extract-text");
  ...
  if (text.length > 0) {
    ...
    const { setDocumentContent } = await import("@/client/lib/demo/document-content-cache");
